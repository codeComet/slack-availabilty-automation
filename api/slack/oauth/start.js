...
module.exports = async function handler(req, res) {
  ...
  // If state is already provided (from /api/connect "add workspace" flow), use it as-is.
  // Otherwise build it from slack_user_id + team_id (initial connection from /availability command).
  let state = req.query?.state
  let forceTeamId = null   // set only for initial connection
  ...
  if (!state) {
    ...
    forceTeamId = teamId || null   // force the workspace the command came from
  }
  // For the /api/connect flow (state has email), forceTeamId stays null so
  // Slack shows a workspace picker instead of locking to one workspace.
  ...
  if (forceTeamId) oauthParams.team = forceTeamId
